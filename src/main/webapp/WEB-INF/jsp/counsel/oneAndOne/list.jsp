...
<%@ include file="/WEB-INF/jsp/common/inc.jsp" %>
    <script>
    	...
    	function fnGoCounselView(seq) {
    		$("#condSeq").val(seq);
    		$("#frm").attr('action', "${pageContext.request.contextPath}/cpes/all/counsel/view.do");
    		$("#frm").submit();
    	}
	    	...
	    	<header class="sub_head">
	       	...
	       	<div id="contents" class="counselList">
				<div class="bbs_info clearfix tab_confirm"><!-- 서브페이지에 탭 메뉴가 있을 경우 클래스 tab_confirm 추가  -->
					<form id="frm" name="frm" action="${pageContext.request.contextPath}/cpes/all/counsel/list.do" method="post">
				...
				<div class="bbs_basic">
					<ul class="bbs_list clearfix"><!-- 리스트 페이지에서 첨부파일 다운로드 기능이 있는 경우 클래스 has_attachment 추가, 리스트 페이지에서 상태 표시 있을 경우 클래스 has_status 추가 -->
								...
								<div class="title_box <c:if test="${data.newYn == 'Y'}">new</c:if>"><!-- 새 게시물일 경우 클래스  new 추가 -->
									...
									<c:choose>
										<c:when test="${data.secretYn == 'Y'}">
											<span class="bbs_statuses type01"><spring:message code="counsel.msg.openYn.n"/>
										</c:when>
										<c:otherwise>
											<span class="bbs_statuses type02"><spring:message code="counsel.msg.openYn.y"/>
										</c:otherwise>
									</c:choose>
									</span>
						...
						<c:if test="${empty resultList}">
						<li><div class="contents_box"><spring:message code="counsel.msg.no.data"/></div></li>
						</c:if>
				...
				<div class="bbs_btn_wrap clearfix">
					<span class="bbs_right">
						<a href="javascript:fnGoWrite('frm','${pageContext.request.contextPath}/cpes/all/counsel/write.do');" class="bbs_btn write"><spring:message code="button.create"/></a>
					</span>
				</div>
				<!-- //bbs_btn_wrap -->
