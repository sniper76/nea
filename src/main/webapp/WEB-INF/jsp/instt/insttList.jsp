...
<%@ include file="/WEB-INF/jsp/common/inc.jsp" %>
    ...
    var pageSize = 10;
    	...
    	function fnSetAdvanceSrch() {
    		...
    		$('div.detail_area select#condOwnerCd').val('');
    	}
    	...
    	function fnGoSrch(page, params) {
			...
			$.ajax({
				type: METHOD_POST,
				url: contextPath + "/instt/insttListAjax.do",
			...
			$('ul.recruitment_list').empty();
       		...
       		for (var k=0; k<lst.length; k++) {
       			...
       			var html = '';
		        	...
		        	html += '		<div class="img_box"><img src="'+contextPath+data.insttLogoFileGrpSeq+'" alt="image"  onerror="fnNoImage(this)" /></div>';
		        	html += '		<div class="contents_box '+data.insttDivClass+'">                               ';
		        	html += '			<div class="title_box '+newClass+'">                                               ';
		        	html += '				<a href="javascript:void(0);" onclick="fnGoView(\''+data.insttSeq+'\');" class="title">'+data.insttNm+'</a>';
		        	html += '			</div>                                                                                                             ';
		        	...
		        	html += '				</span>                                                                                                        ';
		        	html += '				<span class="cont">                                                                                            ';
		        	html += '					<span class="con">'+data.insttAddrFullNm+'</span>                                       ';
		        	html += '					<span class="con"><a href="tel:000000000" class="mobile_phone">'+data.insttOfficeTel+'</a></span>';
		        	html += '				</span>                                                                                                        ';
		        	html += '			</div>                                                                                                             ';
		        	html += '			<div class="other_box">                                                                                            ';
		        	html += '				<span class="top_box">                                                                                         ';
		        	html += '					<span class="date">'+data.insttStsNm+'</span>                                                              ';

		        	html += '<sec:authorize access="hasAnyRole('ROLE_USER,ROLE_STDIT')">';
		        	html += '<span id="bkmkSapn_'+k+'"><button type="button" id="btnBkmk_'+k+'" onclick="fnBkmkType2(\''+data.bkmkSeq+'\',\''+data.insttSeq+'\',LIKE_CATEGORY_INSTT, \'btnBkmk_'+k+'\', \'bkmkSapn_'+k+'\');" class="interest '+onClass+'">interest</button>';
		        	html += '</span>';
		        		html += '</sec:authorize>';
		        	html += '				</span>                                                                                                        ';
		        	html += '			</div>                                                                                                             ';
	...
	<main class="colgroup" id="colgroup">
            <article>
                <header class="sub_head">
                    ...
                    <div class="sub_title">
						<h2><span><spring:message code="eduTrnng.free.search.title7"/></span></h2>
					</div>
                </header>

                <div id="contents" class="insttInsttList">
					...
					<div class="srch_box">
						...
						<div class="detail_area">
							<div class="box_wrap">
					<form id="frm" name="frm" action="${pageContext.request.contextPath}/instt/view.do" method="post">
									...
									<fieldset>
										...
										<table class="bbs_table">
											...
											<tr>
												...
												<td>
													<label for="condKeyword" class="skip"><spring:message code="compny.vacancy.msg.title64"/></label>
											...
											<tr>
												<th scope="col"><spring:message code="eduTrnng.free.search.title6"/></th>
												<td>
													<label for="condOwnerCd" class="skip"><spring:message code="eduTrnng.free.search.title6"/></label>
													<select id="condOwnerCd" name="condOwnerCd">
													<option value=""><spring:message code="counsel.msg.all"/></option>
													<c:forEach var="data" items="${insttOwnerCd}" varStatus="status">
											...
											<tr>
												<th scope="col"><spring:message code="mypage.instt.eduTrnng.title28"/></th>
												<td>
													<label for="condInsttuCd" class="skip"><spring:message code="mypage.instt.eduTrnng.title28"/></label>
