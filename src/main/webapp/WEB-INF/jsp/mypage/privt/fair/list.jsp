...
<%@ include file="/WEB-INF/jsp/common/inc.jsp" %>
    <script>
    	$(document).ready(function() {
      	...
      	function fnCancelFair(seq,frmId) {

    		alertify.confirm("<spring:message code="mypage.private.fair.errors.msg"/>", function (e) {
    			if (e) {
    				$.ajax({
    					url: contextPath + "/cpes/private/fair/cancelProcessAjax.do",
    					type: 'post',
    					data: {
    						condSeq : seq
    					},
      	...
      	function fnDeleteFair() {
    		...
    		alertify.confirm("<spring:message code="compny.vacancy.msg.btn.delete"/>", function (e) {
    			if (e) {
    				$.ajax({
    					url: contextPath + "/cpes/private/fair/deleteProcessAjax.do",
    					type: 'post',
    					data: {
	...
	<main class="colgroup" id="colgroup">
		<article>
	    	<header class="sub_head">
				<div class="sub_title">
					<h2>
						<span><spring:message code="mypage.private.fair.title"/></span>
					</h2>
				</div>
	      	</header>

	       	<div id="contents" class="privateFairList">
				<div class="bbs_info clearfix"><!-- 서브페이지에 탭 메뉴가 있을 경우 클래스 tab_confirm 추가  -->
					<form id="frm" name="frm" action="${pageContext.request.contextPath}/cpes/private/fair/list.do" method="post">
					...
					<c:forEach var="data" items="${resultList}" varStatus="status">
						<li>
							<span class="check_box">
								...
								<input type="checkbox" id="chk_${status.count}" name="chk" value="${data.fairSeq}" />
							</span>
								...
								<div class="contents_box ${fairDivCdClass}"><!-- 전국일 경우 클래스 ncpf , 지방일 경우 클래스 pcpf 추가, 고용포럼일 경우 클래스 ep 추가, 모바일일 경우 클래스 mj 추가, 관련일 경우 클래스 rf 추가 -->
									...
									<div class="other_box type2">
										<span class="top_box">
											...
											<c:choose>
												<c:when test="${data.jcAgreeStsCd == 'JAS0000000001'}"><!-- 승인 -->
													<span class="registering">${data.jcAgreeStsNm}</span>
												</c:when>
												<c:otherwise>
													<span class="close">${data.jcAgreeStsNm}</span><!--대가,반려 -->
												</c:otherwise>
											</c:choose>
										</span>
										<span class="bottom_box">
											...
											<c:choose>
												<c:when test="${data.cancelYn == 'Y'}"><spring:message code="mypage.compny.applic.msg11"/></c:when>
												<c:otherwise>
													<button type="button" onclick="fnCancelFair('${data.fairSeq}','frm');" class="bbs_btn small"><spring:message code="mypage.compny.applic.msg11"/></button>
												</c:otherwise>
											</c:choose>
				...
				<div class="bbs_btn_wrap clearfix">
					...
					<span class="bbs_right">
						<button type="button" onclick="fnDeleteFair();" class="bbs_btn delete"><spring:message code="button.delete"/></button>
					</span>
				</div>
				<!-- //bbs_btn_wrap -->
