...
<%@ include file="/WEB-INF/jsp/common/inc.jsp" %>
    <script>
    	$(document).ready(function() {
      	...
      	function fnInviteOk(seq,seq2,frmId) {

    		alertify.confirm("<spring:message code="mypage.compny.fair.msg"/>", function (e) {
    			if (e) {
    				$.ajax({
    					url: contextPath + "/cpes/compny/fair/inviteOkProcessAjax.do",
    					type: 'post',
    					data: {
    						condSeq : seq,
    						condSeq2 : seq2
      	...
      	function fnDeleteInvite(seq,seq2,frmId) {
	...
	<main class="colgroup" id="colgroup">
		<article>
	    	<header class="sub_head">
				<div class="sub_title">
					<h2>
						<span><spring:message code="mypage.compny.fair.title"/></span>
					</h2>
				</div>
	      	</header>
	       	...
	       	<div id="contents" class="fairInviteList">
				<div class="bbs_info clearfix tab_confirm"><!-- 서브페이지에 탭 메뉴가 있을 경우 클래스 tab_confirm 추가  -->
					<form id="frm" name="frm" action="${pageContext.request.contextPath}/cpes/compny/fair/inviteList.do" method="post">
								...
								<div class="contents_box ${fairDivCdClass}"><!-- 전국일 경우 클래스 ncpf , 지방일 경우 클래스 pcpf 추가, 고용포럼일 경우 클래스 ep 추가, 모바일일 경우 클래스 mj 추가, 관련일 경우 클래스 rf 추가 -->
									...
									<div class="other_box type2">
										...
										<span class="bottom_box">
											<c:choose>
												<c:when test="${data.compnyRespYn != 'Y'}">
													<button type="button" onclick="fnInviteOk('${data.compnySeq}','${data.fairSeq}','frm');" class="bbs_btn type01 small"><spring:message code="mypage.compny.applic.msg3"/></button>
												</c:when>
												<c:otherwise>
													${data.jcAgreeStsNm}
												</c:otherwise>
				...
				<div class="bbs_btn_wrap clearfix">
					...
					<span class="bbs_right">
						<button type="button" onclick="fnDeleteInvite();" class="bbs_btn delete"><spring:message code="button.delete"/></button>
					</span>
				</div>
				<!-- //bbs_btn_wrap -->
