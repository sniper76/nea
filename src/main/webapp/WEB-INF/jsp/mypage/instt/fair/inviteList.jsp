    	...
    	$(document).ready(function() {
      	...
      	function fnInviteOk(seq,seq2,frmId) {
    		...
    		alertify.confirm("<spring:message code="mypage.compny.fair.msg"/>", function (e) {
    			if (e) {
    				$.ajax({
    					url: contextPath + "/cpes/instt/fair/inviteOkProcessAjax.do",
    					type: 'post',
    					data: {
    						condSeq : seq,
    						condSeq2 : seq2
	    	...
	    	<header class="sub_head">
	       	...
	       	<div id="contents" class="fairInviteList">
				<div class="bbs_info clearfix tab_confirm"><!-- 서브페이지에 탭 메뉴가 있을 경우 클래스 tab_confirm 추가  -->
					<form id="frm" name="frm" action="${pageContext.request.contextPath}/cpes/instt/fair/inviteList.do" method="post">
								...
								<div class="contents_box ${fairDivCdClass}"><!-- 전국일 경우 클래스 ncpf , 지방일 경우 클래스 pcpf 추가, 고용포럼일 경우 클래스 ep 추가, 모바일일 경우 클래스 mj 추가, 관련일 경우 클래스 rf 추가 -->
									...
									<div class="other_box type2">
										...
										<span class="bottom_box">
											...
											<c:choose>
												<c:when test="${data.insttRespYn != 'Y'}">
													<button type="button" onclick="fnInviteOk('${data.insttSeq}','${data.fairSeq}','frm');" class="bbs_btn type01 small"><spring:message code="button.request"/></button>
												</c:when>
												<c:otherwise>
													 ${data.jcAgreeStsNm}
												</c:otherwise>
