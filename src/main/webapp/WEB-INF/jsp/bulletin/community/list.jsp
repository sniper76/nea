...
<%@ include file="/WEB-INF/jsp/common/inc.jsp" %>
    ...
    <script>
    	$(document).ready(function() {
       	...
       	function fnGoCommunityView(seq) {
       		$("#condSeq").val(seq);
       		$("#frm").attr('action', "${pageContext.request.contextPath}/community/view.do");
       		$("#frm").submit();
       	}
	...
	<main class="colgroup" id="colgroup">
		<article>
	    	<header class="sub_head">
	       	...
	       	<div id="contents" class="communityList">
				<div class="bbs_info clearfix tab_confirm"><!-- 서브페이지에 탭 메뉴가 있을 경우 클래스 tab_confirm 추가  -->
					<form id="frm" name="frm" action="${pageContext.request.contextPath}/community/list.do" method="post">
						...
						<li>
							<div class="contents_box <c:if test="${data.ntcYn == 'Y'}">notice</c:if>">
								<div class="title_box <c:if test="${data.newYn == 'Y'}">new</c:if>"><!-- 새 게시물일 경우 클래스  new 추가 -->
									<a href="javascript:fnGoCommunityView('${data.bulletinSeq}')" class="title"><c:out value="${data.title}" escapeXml="false"/>
										<c:if test="${result.commentYn == 'Y' && data.commentCnt > 0}">
										...
										<c:if test="${result.likeYn == 'Y'}"><!-- 게시판설정에 좋아요가 있을경우 -->
											...
											<c:choose>
												...
												<c:when test="${data.likeYn == 'Y'}">
													<span class="btn_like send" onclick="javascript:fnLike(LIKE_CATEGORY_COMMUNITY,'${data.bulletinSeq}','likeSpan_${status.count}','likeCnt_${status.count}', '${data.likeSeq}');"><spring:message code="counsel.msg.reg.like.cnt"/></span>
												</c:when>
												<c:when test="${data.likeYn == 'N'}">
													<button class="btn_like" onclick="javascript:fnLike(LIKE_CATEGORY_COMMUNITY,'${data.bulletinSeq}','likeSpan_${status.count}','likeCnt_${status.count}');"><spring:message code="counsel.msg.reg.like.cnt"/></button>
												</c:when>
											</c:choose>
											</span>
										</c:if>
				...
				<div class="bbs_btn_wrap clearfix">
					<span class="bbs_right">
						<a href="javascript:fnGoMoveUrl('frm','${pageContext.request.contextPath}/community/write.do');" class="bbs_btn write"><spring:message code="button.create"/></a>
					</span>
				</div>
				<!-- //bbs_btn_wrap -->
