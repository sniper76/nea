						...
						<c:if test="${result.likeYn == 'Y'}"><!-- 게시판설정에 좋아요가 있을경우 -->
							...
							<c:choose>
								<c:when test="${loginYn == 'N'}">
									<span class="btn_like close" id="btnLike"><spring:message code="counsel.msg.reg.like.cnt"/></span>
								</c:when>
								<c:when test="${result2.likeYn == 'Y'}">
									<span class="btn_like send" id="btnLike" onclick="javascript:fnLike(LIKE_CATEGORY_BULLETIN,'${result2.bulletinSeq}','likeSpan','likeCnt', '${result2.likeSeq}');"><spring:message code="counsel.msg.reg.like.cnt"/></span>
								</c:when>
								<c:when test="${result2.likeYn == 'N'}">
									<button class="btn_like" id="btnLike" onclick="javascript:fnLike(LIKE_CATEGORY_BULLETIN,'${result2.bulletinSeq}','likeSpan','likeCnt');"><spring:message code="counsel.msg.reg.like.cnt"/></button>
								</c:when>
							</c:choose>
							</span>
						</c:if>
