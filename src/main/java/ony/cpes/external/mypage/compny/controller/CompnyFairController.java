...
@RequestMapping("/cpes/compny/fair")
@Controller
public class CompnyFairController extends BaseController{
	private static Log log = LogFactory.getLog(CompnyFairController.class);

	@Autowired
	private CommonService commonService;

	@Autowired
	private CompnyFairService compnyFairService;
	 ...
	 */
	  ...
	  public ModelAndView inviteList(Locale locale,
		...
		mv.setViewName("mypage/compny/fair/inviteList.left");
		...
		condFairBean.setCondUserSeq(SessionUtil.getUserSeq(req));
      	...
      	int totCnt = compnyFairService.selectInviteListCnt(condFairBean);

      	paginationInfo.setTotalRecordCount(totCnt);
    	mv.addObject(ConstVal.PAGINATION_INFO_KEY, paginationInfo);

    	if(totCnt > 0) {
    		mv.addObject(ConstVal.RESULT_LIST_KEY, compnyFairService.selectInviteList(condFairBean));
    	}
	...
	public ModelAndView inviteOkProcessAjax(Locale locale,
		...
		condFairBean.setCondUserSeq(userSeq);
  		...
  		if(compnyFairService.updateInviteOk(condFairBean) > 0) {
	...
	public ModelAndView inviteDeleteProcessAjax(Locale locale,
		...
		condFairBean.setLangCd(locale.getLanguage().toUpperCase());//언어코드,lanuage code
  		...
  		if(compnyFairService.deleteInvite(condFairBean) > 0) {
  			ajaxResultBean.setSuccessYn(ConstVal.YES_VAL);//입력실패,insert fail
      		ajaxResultBean.setStatCd(ConstVal.STAT_CD_SUCCESS_VAL);
  		} else {
  			ajaxResultBean.setSuccessYn(ConstVal.NO_VAL);//입력실패,insert fail
	 ...
	 */
	  ...
	  public ModelAndView joinList(Locale locale,
		...
		ModelAndView mv = new ModelAndView();
		mv.setViewName("mypage/compny/fair/joinList.left");
		...
		condFairBean.setCondUserSeq(SessionUtil.getUserSeq(req));
      	...
      	int totCnt = compnyFairService.selectJoinListCnt(condFairBean);
      	paginationInfo.setTotalRecordCount(totCnt);
    	mv.addObject(ConstVal.PAGINATION_INFO_KEY, paginationInfo);

    	if(totCnt > 0) {
    		mv.addObject(ConstVal.RESULT_LIST_KEY, compnyFairService.selectJoinList(condFairBean));
    	}
	 ...
	 */
	  ...
	  public ModelAndView vacancyList(Locale locale,
		...
		ModelAndView mv = new ModelAndView();
		mv.setViewName("mypage/compny/fair/vacancyList.left");


		condFairBean.setLangCd(locale.getLanguage().toUpperCase());//언어코드,lanuage code
		condFairBean.setCondUserSeq(SessionUtil.getUserSeq(req));

      	mv.addObject(ConstVal.RESULT_KEY, compnyFairService.selectFair(condFairBean));
	    mv.addObject(ConstVal.RESULT_LIST_KEY, compnyFairService.selectVacancyList(condFairBean));
	 ...
	 */
	@SuppressWarnings("unused")
	@RequestMapping(value = "/inviteCancelProcessAjax", method = RequestMethod.POST)
	public ModelAndView inviteCancelProcessAjax(Locale locale,
		...
		condFairBean.setCondUserSeq(userSeq);
  		...
  		if(compnyFairService.updateInviteCancel(condFairBean) > 0) {
  			ajaxResultBean.setSuccessYn(ConstVal.YES_VAL);//입력실패,insert fail
      		ajaxResultBean.setStatCd(ConstVal.STAT_CD_SUCCESS_VAL);
  		} else {
  			ajaxResultBean.setSuccessYn(ConstVal.NO_VAL);//입력실패,insert fail
	 ...
	 */
	@SuppressWarnings("unused")
	@RequestMapping(value = "/deleteFairVacancyMultiAjax", method = RequestMethod.POST)
	public ModelAndView deleteFairVacancyMultiAjax(Locale locale,
				@ModelAttribute("ConditionBean") ConditionBean conditionBean,
				@ModelAttribute("CondFairBean") CondFairBean condFairBean,
				Principal principal,
				HttpServletRequest req,
		...
		condFairBean.setCondUserSeq(userSeq);
  		...
  		if(compnyFairService.deleteVacancyMulti(condFairBean) > 0) {
  			ajaxResultBean.setSuccessYn(ConstVal.YES_VAL);//입력실패,insert fail
      		ajaxResultBean.setStatCd(ConstVal.STAT_CD_SUCCESS_VAL);
  		} else {
  			ajaxResultBean.setSuccessYn(ConstVal.NO_VAL);//입력실패,insert fail
	 ...
	 */
	  ...
	  public ModelAndView boothList(Locale locale,
		...
		ModelAndView mv = new ModelAndView();
		mv.setViewName("mypage/compny/fair/boothList.left");
		...
		condFairBean.setCondUserSeq(SessionUtil.getUserSeq(req));
      	...
      	int totCnt = compnyFairService.selectBoothListCnt(condFairBean);
      	paginationInfo.setTotalRecordCount(totCnt);
    	mv.addObject(ConstVal.PAGINATION_INFO_KEY, paginationInfo);

    	if(totCnt > 0) {
    		mv.addObject(ConstVal.RESULT_LIST_KEY, compnyFairService.selectBoothList(condFairBean));
    	}
	...
	public ModelAndView boothCancelProcessAjax(Locale locale,
		...
		condFairBean.setLangCd(locale.getLanguage().toUpperCase());//언어코드,lanuage code
  		...
  		if(compnyFairService.updateBoothCancel(condFairBean) > 0) {
  			ajaxResultBean.setSuccessYn(ConstVal.YES_VAL);//입력실패,insert fail
      		ajaxResultBean.setStatCd(ConstVal.STAT_CD_SUCCESS_VAL);
  		} else {
  			ajaxResultBean.setSuccessYn(ConstVal.NO_VAL);//입력실패,insert fail
